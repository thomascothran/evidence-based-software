...
Our approach is both practical and rigorous. We start with concrete, real-world problems faced by software development teams. We then introduce the mathematical and theoretical principles that can solve these problems, illustrating these principles with specific, tangible examples. We provide detailed, step-by-step instructions for implementing these solutions, ensuring you can apply these principles directly to your work.
...
[**Monte Carlo Forecasting**](./monte_carlo_forecasting.clj)
...
[***Bayes Theorem**](./bayes_theorem.clj). Problem: how can you collect, evaluate, and make intelligent decisions based on your own data? Sampling and the use of Bayes theorem.

[**What work items to work on?**](./what_work_items_to_work_on.clj) Problem: How do we manage what work items to start? Work items are smaller than the initiative level, these are stories.

[**Predictability**](./predictability.clj) Problem: how can we improve our predictability, and reduce our variability, understanding that the reduction in variability must be made in an economically informed way.

[**Process Modelling with Monte Carlo**](./monte_carlo_process_modeling.clj). Problem: how can we use Monte Carlo to model how work items flow through our value stream, and identify the areas which are most beneficial to optimize?

[**Process Modeling with Agent Based Models**](./agent_based_models.clj) Problem: how can we use agent based models to simulate how work items flow through our value stream, and identify the areas that are most beneficial to optimize?
")
